...
function neofetchOutput() {
    ...
    const row = (colors) => colors.map(c => `<span class="swatch" style="background:${c}"></span>`).join('');
...
const commands = {
    ...
    help: () => '<div>Available commands: <span class="c-arch">neofetch</span>, ' +
...
function handleCommand(raw) {
    ...
    if (cmd === '')
        return;
    appendOutput(`<div><span class="c-arch">arch2kx@archlinux ~ %</span> ${escapeHtml(cmd)}</div>`);
    if (cmd === 'clear') {
        output.innerHTML = '';
        return;
    }
    ...
    if (fn !== undefined) {
        ...
        appendOutput(`<div><span class="c-err">zsh: command not found: ${escapeHtml(cmd)}</span></div>`);
    }
}
